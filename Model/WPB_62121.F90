...
! as was implemented in the paper "Insert title here" (2021)
...
subroutine BeetleFit(Length,Tminvec, Tmaxvec,r1,x0,x1,CWDvec,x2,SizeFactor,NtGEQ317,NtGEQ00,Tparents,NGEQ20vec,NGEQ00vec,&
    ...
    do i = 2,TrueLength
        ...
        if(i== 1 .or. i ==  1+365 .or. i == 1+2*365 .or. i ==  1+3*365 .or. &
			...
			i ==  1+17*365) then
        FebInPopn = Fec + E + (L1 + L2 )/(1.0 + exp(-(ColdestT + 28.391)/6.124)) + P + Te + A
...
Subroutine BeetleSim(Tmax, Tmin,TminNext, Parents, FA, OE, OL1, OL2, &
	...
	!real(kind = 8), parameter :: sigma0 = 0.0001         ! controls rate variability
   ...
   ! parameters for development rate for eggs
	...
	!real(kind = 8), parameter :: sigma2=.00001        
	...
	!real(kind = 8), parameter :: sigma3=.00001 
    ...
    !real(kind = 8), parameter ::sigma4=0.0001
...
subroutine FlightFunc(TC, Flying)
    ...
    if(TC >= 18.6 .and. TC <= 38.9) then
        Flying = -1.2987347+ (0.1090086)*(TC)+( -0.0019475)*(TC**2.0)
    end if
